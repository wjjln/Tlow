...
import requests
import tiktoken
from typing import Union, Optional, List, Dict, Any, DefaultDict
import logging
...
def group_keys_by_frequency(
    keys: List[Any], 
    key2num: DefaultDict[Any, int], 
    group_cut: List[int]
):
    ...
    group_labels = []
    last_cut = -1
    for cut in group_cut:
        if cut < last_cut:
             raise ValueError(f"group_cut {cut} <= {last_cut}")
        if cut == 0:
            group_labels.append(f"0")
        else:
            group_labels.append(f"[{last_cut + 1}, {cut}]")
        last_cut = cut
    group_labels.append(f">{last_cut}")

    grouped_keys = dict()

    for key in keys:

        frequency = key2num[key]
        group_index = bisect.bisect_left(group_cut, frequency)
        # >>> bisect.bisect_right([0, 5, 10, 15], 0)
        # 1
        # >>> bisect.bisect_left([0, 5, 10, 15], 0)
        # 0
        # >>> bisect.bisect_left([0, 5, 10, 15], 1)
        # 1
        # >>> bisect.bisect_left([0, 5, 10, 15], 10)
        # 2
        # >>> bisect.bisect_left([0, 5, 10, 15], 5)
        # 1
