...
def extract_codebooks_from_faiss_index(index_path):
    try:
        ...
        if "IVFPQ" not in str(type(ivf_index)):
             return None

        pq = ivf_index.pq

        centroids_flat = faiss.vector_to_array(pq.centroids)
        codebooks = centroids_flat.reshape(pq.M, pq.ksub, pq.dsub)
...
def calc_cos_sim(input_tensor):
    normalized_tensor = F.normalize(input_tensor, p=2, dim=-1)
    similarity_matrix = torch.matmul(normalized_tensor, normalized_tensor.transpose(-1, -2))
    return similarity_matrix

class Tlow(AbstractModel):
    def __init__(
        self,
        config: dict,
        ...
        super(Tlow, self).__init__(config, dataset, tokenizer)
        ...
        if self.config['sim_loss_weight'] > 0:
            import os
            index_path = os.path.join(
                dataset.cache_dir, 'processed',
                f'{os.path.basename(self.config["sent_emb_model"])}_Tlow_IVF1,PQ{self.config["n_codebook"]}x{tokenizer.n_codebook_bits}.faissindex'
            )
            codebooks = extract_codebooks_from_faiss_index(index_path)
            self.codebooks = torch.from_numpy(codebooks).to(self.config['device'])
            self.z_sim = calc_cos_sim(self.codebooks)
        ...
        if self.config['token_agg'] == 'att':
            self.agg = nn.MultiheadAttention(embed_dim=self.config['n_embd'], num_heads=self.config['agg_n_head'], batch_first=True)
            self.agg_layer_norm = nn.LayerNorm(self.config['n_embd'])
        ...
        self.rng_generator = torch.Generator(device=self.config['device'])
        self.rng_generator.manual_seed(self.config['rand_seed'])
    ...
    def forward(self, batch: dict, return_loss=True) -> torch.Tensor:
        ...
        if return_loss:
            ...
            if self.config['sim_loss_weight'] > 0:
                token_sim = calc_cos_sim(token_emb.view(self.config['n_codebook'], self.config['codebook_size'], self.config['n_embd']))
                sim_loss = F.mse_loss(token_sim, self.z_sim)
                outputs.sim_loss = sim_loss
                outputs.loss = outputs.loss + self.config['sim_loss_weight']*sim_loss
        return outputs
