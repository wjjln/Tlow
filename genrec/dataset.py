...
class AbstractDataset:
    ...
    def _leave_one_out(self):
        ...
        if self.config["dataset"] in ['Douban', 'ClothSport']:
            ...
            for user in self.all_domain_seqs:
                datasets['test']['domain'].append(self.all_domain_seqs[user][-1])
                if len(self.all_domain_seqs[user]) > 1:
                    datasets['val']['domain'].append(self.all_domain_seqs[user][-2])
